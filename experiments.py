...
def plot_experiment_results(experiment_name: str, results: List[Tuple[List[P.Polynomial], List[float], int]]):
    ...
    plt.fill_between(range(len(mean_losses)), mean_losses - std_losses, mean_losses + std_losses, color='red', alpha=0.2)
...
def test_adam_vs_sgd():
    adam_results = run_experiment("adam", 10, use_adam=True, max_iter=5000, verbose=False, plot=False)
    sgd_results = run_experiment("sgd", 10, use_adam=False, max_iter=5000, verbose=False, plot=False)
    
    plot_experiment_results("adam", adam_results)
    plot_experiment_results("sgd", sgd_results)

def test_multiple_initializations():
    results = run_experiment("multiple_initializations", 20, max_iter=5000, verbose=False, plot=False)
    plot_experiment_results("multiple_initializations", results)

def test_polynomial_degree_limits():
    degrees = [3, 9, 27, 81, 243]
...
def test_known_composition():
    def create_known_target():
        p1 = P.Polynomial([0, 2, -1])  # 2x - x^2
        p2 = P.Polynomial([1, 3])     # 1 + 3x
        p3 = P.Polynomial([0.5, -2, 0, 1])  # .5 - 2x + x^3
        return compose_layers([p1, p2, p3])

    target = create_known_target()
    results = run_experiment("known_composition", 10, target=target, random_initialization_deg=[2, 1, 3], max_iter=10000, verbose=False, plot=False)
...
def test_hyperparameters():
    ...
    for bs in batch_sizes:
        results = run_experiment(f"batch_size_{bs}", 5, batch_size=bs, max_iter=5000, verbose=False, plot=False)
        plot_experiment_results(f"batch_size_{bs}", results)

    # Test learning rate
    learning_rates = [0.001, 0.01, 0.05, 0.1, 0.5]
    for lr in learning_rates:
        results = run_experiment(f"learning_rate_{lr}", 5, lr=lr, max_iter=5000, verbose=False, plot=False)
    ...
    for beta1, beta2 in adam_betas:
        results = run_experiment(f"adam_betas_{beta1}_{beta2}", 5, use_adam=True, beta1=beta1, beta2=beta2, max_iter=5000, verbose=False, plot=False)
        plot_experiment_results(f"adam_betas_{beta1}_{beta2}", results)

    # Test use_scale_lr
    for use_scale in [True, False]:
        results = run_experiment(f"use_scale_lr_{use_scale}", 10, use_scale_lr=use_scale, max_iter=5000, verbose=False, plot=False)
        plot_experiment_results(f"use_scale_lr_{use_scale}", results)

if __name__ == "__main__":
    test_adam_vs_sgd()
    # test_multiple_initializations()  # TODO: fix later
