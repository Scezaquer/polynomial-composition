...
def test_adam_vs_sgd():
    adam_results = run_experiment("adam", 10, use_adam=True, max_iter=25000, verbose=False, plot=False)
    sgd_results = run_experiment("sgd", 10, use_adam=False, max_iter=25000, verbose=False, plot=False)
...
def test_multiple_initializations():
    ...
    results = run_experiment("multiple_initializations", 10, target=target, max_iter=25000, verbose=False, plot=False)
...
def test_polynomial_degree_limits():
    ...
    for i, deg in enumerate(degrees):
        results = run_experiment(f"degree_limit_{deg}", 5, random_target_poly_deg=deg, random_initialization_deg=[3]*(i+1), max_iter=25000, verbose=False, plot=False)
...
def test_component_count():
    ...
    for init_deg, target_deg in configurations:
        results = run_experiment(f"component_count_{len(init_deg)}_{target_deg}", 10, random_initialization_deg=init_deg, random_target_poly_deg=target_deg, max_iter=25000, verbose=False, plot=False)
...
def test_known_composition():
    ...
    results = run_experiment("known_composition", 10, target=target, max_iter=25000, verbose=False, plot=False)
    plot_experiment_results("known_composition", results)


def test_hyperparameters():
    # Test batch size
    batch_sizes = [20, 100, 500, 2000, 5000]
    for bs in batch_sizes:
        results = run_experiment(f"batch_size_{bs}", 5, batch_size=bs, max_iter=25000, verbose=False, plot=False)
        plot_experiment_results(f"batch_size_{bs}", results)

    # Test learning rate
    learning_rates = [0.001, 0.002, 0.005, 0.01, 0.05]
    for lr in learning_rates:
        results = run_experiment(f"learning_rate_{lr}", 5, lr=lr, max_iter=25000, verbose=False, plot=False)
    ...
    for beta1, beta2 in adam_betas:
        results = run_experiment(f"adam_betas_{beta1}_{beta2}", 5, use_adam=True, beta1=beta1, beta2=beta2, max_iter=25000, verbose=False, plot=False)
        plot_experiment_results(f"adam_betas_{beta1}_{beta2}", results)

    # Test use_scale_lr
    for use_scale in [True, False]:
        results = run_experiment(f"use_scale_lr_{use_scale}", 10, use_scale_lr=use_scale, max_iter=25000, verbose=False, plot=False)
